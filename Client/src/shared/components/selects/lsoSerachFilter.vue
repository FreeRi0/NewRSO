<template>
    <v-autocomplete v-model="selected" v-if="props.SortDropdown" :items="items" chips clearable
        v-model:search.trim="name" variant="outlined" item-title="name" item-value="value" v-bind="$attrs"
        @keyup="searchLSO" @update:modelValue="changeValue" :address="addressRef" :no-data-text="noDataText"
        class="option-select">
    ...
    <v-autocomplete v-model="selected" :items="items" chips clearable v-else v-model:search.trim="name"
        variant="outlined" item-title="name" item-value="id" v-bind="$attrs" @keyup="searchEducInstitution"
        @update:modelValue="changeValue" :address="addressRef" :no-data-text="noDataText" class="option-select">
        <template #prepend-inner v-if="changeUser">
            <Icon icon="clarity-search-line" color="#222222" width="24" height="24" class="option-select__icon mr-3">
            </Icon>
        </template>
...
const emit = defineEmits(['update:value', 'searchDetachment']);
...
// const searchSquads = async (name) => {
//     const searchSquadsResp = await HTTP.get(
//         `/detachments/?search=${name}`,
//     );
//     items.value = searchSquadsResp.data.results;
// }
...
const searchLSO = (val) => {
    ...
    timer.value = setTimeout(() => {
        // searchSquads(name.value);
        emit('searchDetachment', name.value);
