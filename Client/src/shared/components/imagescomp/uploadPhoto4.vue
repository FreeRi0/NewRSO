...
const cropImage = () => {
    if (cropper.value) {
        ...
        canvas.toBlob((blob) => {
            userPhotos.value.photo4 = new File([blob], "photo1.jpg", { type: "image/jpeg" })
        }, 'image/jpeg');
    }
};

const selectFile = (event) => {
    userPhotos.value.photo4 = event.target.files[0];
    preview.value = URL.createObjectURL(userPhotos.value.photo4);
};

const uploadPhoto = async () => {
    dialog.value = true;
    const formData = new FormData();
    formData.append('photo4', userPhotos.value.photo4);
...
const updatePhoto = async () => {
    let fd = new FormData();
    fd.append('photo4', userPhotos.value.photo4);
    dialog.value = true;
