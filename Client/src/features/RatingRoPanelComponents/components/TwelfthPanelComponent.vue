<template>
  <div class="report__field-group" v-if="!isSecondTab && !isThirdTab">
    <div class="report__fieldset report__fieldset--left-block">
      <label class="form__label report__label" for="number-of-members">
    ...
    >
      ...
      <InputReport
        v-if="
          !twelfthPanelData.scan_file ||
          (isRevision && props.tab !== 'Просмотр отправленного отчета')
        "
        isFile
        type="file"
    ...
    <div class="report__fieldset report__fieldset--left-block">
      <label class="form__label report__label" for="number-of-members">
        Количество членов РО&nbsp;РСО, принявших участие во&nbsp;Всероссийском дне
        ударного труда&nbsp;<sup class="valid-red">*</sup>
      </label>
      <InputReport
  ...
  <div v-if="isThirdTab" class="report__field-group report__field-group--column">
    <ReportTable
      ...
      :disabled="
        ...
        reportStore.reportForCheckCH.twelfth.verified_by_chq !== null
      "
      :is-error-panel="isErrorPanel"
    ></ReportTable>
    ...
    <ReportTable
      ...
      name="twelfthPanelData.number_of_members"
      :dataRH="twelfthPanelData.number_of_members"
      :dataDH="twelfthPanelDataDH.number_of_members"
      v-model:value="twelfthPanelDataCH.number_of_members"
      :maxlength="10"
      :min="0"
      :max="2147483647"
      :step="1"
    ...
    <div>
      <v-checkbox
        v-if="
          centralExpert
            ? !reportStore.reportForCheckCH.twelfth.verified_by_chq
...
const deleteFile = async () => {
  ...
  if (!isErrorFile.value) {
    ...
    try {
      if (isFirstSent.value) {
        let { data: scan_file } = await reportPartTwoService.createReport(
          formData,
          ID_PANEL,
          true
        );
        ...
        let { data: scan_file } = await reportPartTwoService.createReportDraft(
          formData,
          ID_PANEL,
          true
        );
...
const deleteFileCH = async () => {
  ...
  reportStore.reportDataCHFile.twelfth = null;
  if (reportStore.reportForCheckCH.twelfth.central_version) {
    reportStore.reportForCheckCH.twelfth.central_version.scan_file = null;
  }
};
...
const onReturnReport = (event) => {
  ...
  formData.append(
    ...
    reportStore.reportDataCHFile.twelfth ||
      reportStore.reportForCheckCH.twelfth.central_version?.scan_file ||
      ""
  );
...
watchEffect(
  async () => {
    if (props.centralExpert || props.districtExpert) {
      if (reportStore.reportDataCH.twelfth) {
        ...
        twelfthPanelDataCH.value.amount_of_money =
          reportStore.reportDataCH.twelfth.amount_of_money;
        twelfthPanelDataCH.value.number_of_members =
          reportStore.reportDataCH.twelfth.number_of_members;
      }
      if (reportStore.reportDataDH.twelfth) {
        ...
        twelfthPanelDataDH.value.amount_of_money =
          reportStore.reportDataDH.twelfth.amount_of_money;
        twelfthPanelDataDH.value.number_of_members =
          reportStore.reportDataDH.twelfth.number_of_members;
      }
    } else {
      if (props.data) {
        ...
        isFirstSent.value =
          reportStore.isReportReject.twelfth && !props.data.central_version;
      }
    }
    if (props.districtExpert) {
      ...
      if (reportStore.reportForCheckCH.twelfth.verified_by_chq === true) {
        ...
        reportStore.reportForCheckCH.twelfth.rejecting_reasons &&
        !reportStore.reportDataCHFile.twelfth
      ) {
        fileCH.value.name =
          reportStore.reportForCheckCH.twelfth.central_version.scan_file || "";
        fileCH.value.type =
          reportStore.reportForCheckCH.twelfth.central_version.file_type || "";
        fileCH.value.size =
          reportStore.reportForCheckCH.twelfth.central_version.file_size || "";
      } else if (reportStore.reportDataCHFile.twelfth) {
...
watch(twelfthPanelDataCH.value, () => {
  if (props.centralExpert) {
    ...
    if (reportStore.returnReport.twelfth)
      formData.append("reasons[comment]", twelfthPanelDataCH.value.comment);
    emit("getDataCH", formData, Number(ID_PANEL));
  }
});
