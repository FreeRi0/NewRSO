        ...
        <div class="squad-data__contacts-wrapper">
          <div class="squad-data__contacts">
            <div class="squad-data__participant-counter">
              <span>{{ squad.participants_count }} {{ ending }}</span>
            </div>
            <div class="squad-data__social-network">
              <div
                class="squad-data__link-vk"
                v-if="squad.social_vk && squad.social_vk != 'null'"
              >
              ...
              <div
                ...
                v-if="squad.social_tg && squad.social_tg != 'null'"
              >
              ...
              <div class="squad-data__link-share-link">
                ...
                <div class="copy-message" hidden>Ссылка скопирована</div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="Object.keys(applications).length !== 0" class="d-flex">
            <div class="user-data__link mr-2">Заявка на рассмотрении</div>
            <Button
          ...
          <div v-else-if="IsMember" class="user-data__link">Вы участник</div>
        </div>
...
const IsTrusted = computed(() => {
  return props.member.find((item) => item.user.id == userId && item.is_trusted == true);
});
...
const AddApplication = async () => {
  try {
    ...
    const sendResponse = await HTTP.post(`/detachments/${id}/apply/`, data.value);
    swal.fire({
      position: "top-center",
      icon: "success",
      title: "успешно",
