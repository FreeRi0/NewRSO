<template>
  <div class="participants-wrapper__item" v-for="participant in participants" v-if="participants.length > 0">
            <div class="round-img">
                <img :src="'./assets/' + participant.image" alt="logo" />
            </div>
            <div class="container-participants">
                <p class="participants-wrapper__item-name">
                    {{ participant.name }}
                </p>
                <p class="participants-wrapper__item-position">{{ participant.position }}</p>
            </div>
        </div>

    <h2 v-else>Участник не найден...</h2>
</template>
<script setup>
const props = defineProps({
    participants: {
        type: Array,
        required: true,
    },
});
</script>
<style lang="scss" scoped>
.round-img {
    width: 126px;
    height: 126px;
    display: block;
    margin: 0px auto;
}
.participants-wrapper__item {
    margin: 0px auto;
    width: 170px;
    &-name {
        margin-top: 10px;
        margin-bottom: 5px;
        text-align: center;
        font-size: 28px;
        font-family: 'Akrobat';
        color: #1e1e1e;
    }
    &-position {
        text-align: center;
        font-size: 16px;
        font-family: 'Akrobat';
        color: #676767;
    }
}
</style>