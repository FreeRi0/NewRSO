      ...
      <v-expansion-panel-text>
        <div class="bg_form" v-if="correctionTab !== 3">
          <div class="top_line">
            <div class="form__field places">
              ...
              <p class="form__label" v-else-if="!isFirstPart">
                Количество человек, принимавших участие в трудовом проекте в составе ЛСО
                <sup class="valid-red">*</sup>
              </p>
              <InputReport
                @focusout="focusOut"
          ...
          <div class="form__field">
            ...
            <div
              class="form__wrapper"
              v-for="(item, index) in eventData.links"
              :key="index"
            >
              <InputReport
              ...
              <div v-if="!isSentSix && eventData.number_of_members > 0">
                <div
                  class="add_link"
                  @click="addLink(6)"
                  v-if="eventData.links.length === index + 1"
                >
                  + Добавить ссылку
                </div>
          ...
          <div class="form__field">
            ...
            <TextareaReport
              v-model:value="eventData.comment"
              id="comment"
              name="comment"
              :rows="1"
              autoResize
        ...
        <div class="bg_form" v-else>
          ...
          <ReportTable
            v-if="!isFirstPart"
            label="Количество человек, являвшихся членами Штаба трудового проекта"
            class="mb-4"
            name="sixPanelData.hq_members_count"
            :dataRH="data.hq_members_count"
            :dataDH="reportStore.reportDataDH.six[event.id].hq_members_count"
            v-model:value="sixPanelDataCH.hq_members_count"
            :maxlength="10"
            :min="0"
            :max="2147483647"
            :is-error-panel="isErrorPanel"
...
import { ReportTable } from "@entities/RatingRoComponents/components";
...
const sixPanelDataCH = ref(
  reportStore.reportDataCH?.six?.[props.event.id]
    ? reportStore.reportDataCH.six[props.event.id]
    : { number_of_members: null, hq_members_count: null }
);
const eventData = ref({
  number_of_members: 0,
  links: [{ link: "" }],
    ...
    await reportPartTwoService.createMultipleReportDraft(
      {
        ...
        comment: eventData.value.comment,
        is_hq_member: eventData.value.is_hq_member,
        hq_members_count: eventData.value.is_hq_member
          ? eventData.value.hq_members_count
          : null,
      },
      "6",
      props.event.id
...
watchEffect(() => {
  if (props.data && Object.keys(props.data).length) {
    ...
    eventData.value.links =
      Array.isArray(props.data.links) && props.data.links.length
        ? props.data.links
        : [{ link: "" }];
