  ...
  <v-card class="panel-card">
    <v-expansion-panels v-model="panel" class="mb-2">
      ...
      <v-expansion-panel :disabled="disabled">
        <div
          v-if="
            !(
              ...
              (JSON.stringify(reportStore.isReportReject?.six) !== '{}' &&
            ...
            >
              <event-form
                ...
                :is-error-panel="
                  !!(
                    props.isErrorPanel &&
                    props.isErrorPanel[event.id] &&
                    props.isErrorPanel[event.id].error
                  )
                "
                ...
                @formData="formData($event, event.id)"
                @formDataDH="formDataDH($event, event.id)"
                @formDataCH="formDataCH($event, event.id)"
                @error="setError($event, event.id)"
              />
            </div>
          </div>
        </div>
        ...
        <report-tabs
          v-else
          :isReject="reportStore.isReportReject.six && props.tab === 'Доработка'"
        >
            ...
            <div v-if="selectedEvents.length">
              ...
              >
                <event-form
                  ...
                  "
                  :data="
                    props.data && reportStore.reportDataDH.six[event.id]
                      ? reportStore.reportDataDH.six[event.id]
                      : {}
                  "
                  :correction-tab="2"
                  :tab="props.tab"
...
watchEffect(() => {
  selectedEvents.value.forEach((ev) => {
    const id = ev.id;
    const hasData =
      props.data && props.data[id] && Object.keys(props.data[id]).length > 0;
    if (hasData) {
...
const formDataDH = async (reportData, reportNumber) => {
  if (props.districtExpert) {
    ...
    if (reportData.hq_members_count)
      formData.append("hq_members_count", reportData.hq_members_count || "");

    emit("getDataDH", formData, 7, reportNumber);
  }
};
...
const formDataCH = (reportData, reportNumber) => {
  if (props.centralExpert) {
    ...
    emit("getDataCH", reportData, 7, reportNumber);
  }
};
