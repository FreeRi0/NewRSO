<template>
  <div class="container">
    <bannerCreate
      ...
      label="Создать мероприятие" name="createAction" :eventCreate="true" :target="'_blank'"></bannerCreate>

    <h2 class="action-title">Мероприятия</h2>
    <div class="searcher">
      <div class="squads-search">
        <input type="text" id="search" class="squads-search__input events__input" placeholder="Найти мероприятие"
          v-model="text" @input="SearchByInput" />
        <svg class="events_magnifier" width="28" height="28" viewBox="0 0 28 28" fill="none"
      ...
      <div class="settings-container" :class="{ hideSettings: toggleBtn }">
        <v-expansion-panels class="expansion-panels">
          <v-expansion-panel class="settings-body" title="Тип мероприятия">
            <v-expansion-panel-text>
              <div class="settings-radio">
                <input id="format_typeAll" v-model="actionFormSearch.format_type" value="" type="radio" />
                <label for="format_typeAll">Все</label>
              </div>
              <div class="settings-radio">
                <input v-model="actionFormSearch.format_type" value="Онлайн" type="radio" id="online" />
                <label for="online">Онлайн</label>
              </div>
              <div class="settings-radio">
                <input v-model="actionFormSearch.format_type" value="Офлайн" type="radio" id="offline" />
          ...
          <v-expansion-panel class="settings-body" title="Статус мероприятия">
            <v-expansion-panel-text>
              <div class="settings-radio">
                <input id="statusAll" v-model="actionFormSearch.status" value="" type="radio" />
                <label for="statusAll">Все</label>
              </div>
              <div class="settings-radio">
                <input v-model="actionFormSearch.status" value="active" type="radio" id="open" />
                <label for="open">Активные</label>
              </div>
              <div class="settings-radio">
                <input v-model="actionFormSearch.status" value="inactive" type="radio" id="close" />
          ...
          <v-expansion-panel class="settings-body" title="Масштаб">
            <v-expansion-panel-text>
              <div class="flex align-items-center">
                <div class="settings-radio">
                  <input v-model="actionFormSearch.scale" type="radio" value="" id="scaleAll" />
                  <label for="scaleAll" class="ml-2">Все</label>
                </div>
                <div class="settings-radio">
                  <input v-model="actionFormSearch.scale" type="radio" value="Всероссийское" id="All-Russian" />
                  <label for="All-Russian" class="ml-2">Всероссийское</label>
                </div>
                <div class="settings-radio">
                  <input v-model="actionFormSearch.scale" type="radio" value="Окружное" id="District" />
                  <label for="District" class="ml-2">Окружное</label>
                </div>
                <div class="settings-radio">
                  <input v-model="actionFormSearch.scale" type="radio" value="Региональное" id="Regional" />
                  <label for="Regional" class="ml-2">Региональное</label>
                </div>
                <div class="settings-radio">
                  <input v-model="actionFormSearch.scale" type="radio" value="Городское" id="Urban" />
                  <label for="Urban" class="ml-2">Городское</label>
                </div>
                <div class="settings-radio">
                  <input v-model="actionFormSearch.scale" type="radio" value="Мероприятие ОО" id="OOevent" />
                  <label for="OOevent" class="ml-2">Мероприятие ОО</label>
                </div>
                <div class="settings-radio">
                  <input v-model="actionFormSearch.scale" type="radio" value="Отрядное" id="Detachment" />
          ...
          <v-expansion-panel class="settings-body" title="Направление">
            <v-expansion-panel-text>
              <div class="settings-radio">
                <input id="directionAll" v-model="actionFormSearch.direction" type="radio" value="" />
                <label for="directionAll" class="ml-2">Все</label>
              </div>
              <div class="settings-radio">
                <input v-model="actionFormSearch.direction" type="radio" value="Добровольческое" id="Volunteer" />
                <label for="Volunteer" class="ml-2">Добровольческое</label>
              </div>
              <div class="settings-radio">
                <input v-model="actionFormSearch.direction" type="radio" value="Образовательное" id="Educational" />
                <label for="Educational" class="ml-2">Образовательное</label>
              </div>
              <div class="settings-radio">
                <input v-model="actionFormSearch.direction" type="radio" value="Патриотическое" id="Patriotic" />
                <label for="Patriotic" class="ml-2">Патриотическое</label>
              </div>
              <div class="settings-radio">
                <input v-model="actionFormSearch.direction" type="radio" value="Спортивное" id="Sports" />
                <label for="Sports" class="ml-2">Спортивное</label>
              </div>
              <div class="settings-radio">
                <input v-model="actionFormSearch.direction" type="radio" value="Творческое" id="Creative" />
      ...
      <div class="col" style="width: 100%">
        ...
        <div class="cardsContainer">
          <v-row v-if="vertical" class="cardsRow">
            <v-col class="pa-2" v-for="(variant, i) in actionsList" :key="i" cols="auto" @click="goToEvent(variant.id)"
              style="cursor: pointer">
              <v-card class="mx-auto" min-width="280" height="210" :image="variant.banner">
                <div class="cardClock">
                  <div>
                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
                      ...
                      <path
                        d="M8.39045 1.97289C8.48541 1.88429 8.69465 1.806 8.98572 1.75017C9.2768 1.69433 9.63343 1.66406 10.0003 1.66406C10.3672 1.66406 10.7239 1.69433 11.0149 1.75017C11.306 1.806 11.5152 1.88429 11.6102 1.97289"
                        stroke="#35383F" stroke-linecap="round" />
                    </svg>
                  </div>
                  <div>
                    <p>{{ dateHandler(variant.time_data.registration_end_date, variant.time_data.registration_end_time) }}
                    </p>
                  </div>
              ...
              <div class="cardTimeQuantity">
                ...
                <div class="cardQuantity">
                  ...
                  <div style="padding-top: 3px;">
                    <svg class="quantitySvg" xmlns="http://www.w3.org/2000/svg" fill="none">
          ...
          <div v-else class="vertical_list">
            <div class="vertical_list-item" v-for="item in actionsList" :key="item.id" :id="item.id"
              @click="goToEventList">
