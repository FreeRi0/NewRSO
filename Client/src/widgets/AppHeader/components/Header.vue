    ...
    <header class="header">
      ...
      <ul class="header__logo">
        <li class="header__logo-rso">
          <a href="https://трудкрут.рф/" target="_blank">
            <img src="@app/assets/logo/logo-rso-tablet.svg" width="60" height="58" alt="Логотип сайта трудкрут.рф">
          </a>
        </li>

        <li class="header__logo-labor-cool">
          <a href="https://trudkrutshop.ru/" target="_blank">
            <img src="@app/assets/logo/logo-shop-tablet.svg" width="56" height="56" alt="Логотип сайта trudkrutshop.ru">
          </a>
        </li>
      </ul>
      ...
      <nav class="header__nav header__nav--order">        
        ...
        <ul ref="navMenu" class="header__nav-list no-visible">
            ...
            <li class="header__nav-item">
              <a class="header__nav-link" href="#">Полезная информация</a>
            </li>
        </ul>
      </nav>
      ...
      <nav class="header__nav nav-user">
        <div class="nav-user__application-count">
          <a href=""><!--КОЛОКОЛЬЧИК ЭТО ССЫЛКА?? ЧТО ПРОИСХОДИТ ПРИ НАЖАТИИ??-->
        ...
        <div class="nav-user__location">
          ...
          <div class="nav-user__location-container" v-if="show">
            <button type="button" @click="show = !show" class="nav-user__location-close">x</button>
            ...
            <div>
              <Button class="nav-user__button-agree" label="Да, все верно" color="primary" size="large" @click="show = !show"></Button>
              <Button class="nav-user__button-change" label="Выбрать другой" size="large"></Button><!--если переход на другую страницу, то переделать на ссылку-->
            </div>
          </div>
        </div>
        ...
        <div class="nav-user__menu user-menu">
          <Dropdown :items="userPages" :image=true url="/assets/avatar-user.svg" desc="Фотография пользователя"/>          
        </div>
      </nav> 
    </header>
  </div>
...
<style lang="scss">//убрала тег scoped потому что стили к кнопке dropdown не применялись
...
.header__nav-list.no-visible {//пока не прописала логику бургера
  @media(max-width: 1024px) {
    display: none;
  }
}
